...
const CAMERA_MARKERS: MapMarker[] = [
  {
    ...
    type: "camera",
    label: "CAM-042",
    popup: "CG Road / Swastik Cross Roads — Live",
  },
  {
    id: "CAM-018",
    lat: 23.0395,
    ...
    type: "camera",
    label: "CAM-018",
    popup: "SG Highway Junction — Live",
  },
];
...
export default function MapPage() {
  ...
  const [activeIncident, setActiveIncident] = useState<BackendIncident | null>(
    null,
  );
  ...
  const loadIncidentMapData = useCallback(async (incident: BackendIncident) => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/incidents/${incident.id}/map-data`, {
      ...
      if (res.ok) {
        ...
        const affectedFeatures = geojson.features.filter(
          (f) => f.properties?.feature_type === "affected_segment",
        );
        const diversionFeatures = geojson.features.filter(
          (f) => f.properties?.feature_type === "diversion_route",
        );
        const signalFeatures = geojson.features.filter(
        ...
        const routes: RouteOption[] = diversionFeatures
          .slice(0, 5)
          .map((f, i) => ({
            idx: i,
            ...
            label:
              ...
              (f.properties?.route_description as string) ||
              `Route ${i + 1} via ${(f.properties?.road_names as string[])?.[0] ?? "alternate road"}`,
            extra_minutes:
              (f.properties?.estimated_extra_minutes as number) ?? 5 + i * 2,
            redistribution:
              (f.properties?.traffic_redistribution_pct as number) ??
              30 - i * 5,
            waypoints: [],
            selected: i === 0,
          }));
      ...
      const recRes = await fetch(`${BACKEND}/recommendations/${incident.id}`, {
        signal: AbortSignal.timeout(6000),
      });
      if (recRes.ok) {
        const recs: BackendRecommendation[] = await recRes.json();
        const signalActions = recs.flatMap(
          (r) => r.copilot_response?.signal_actions ?? [],
        );
        if (signalActions.length > 0) {
          console.log(
            `[map] ${signalActions.length} signal actions from backend`,
          );
        }
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (activeIncident.location_lat && activeIncident.location_lon) {
      mapHandleRef.current?.flyTo(
        [activeIncident.location_lon, activeIncident.location_lat],
        15,
      );
    }
  ...
  useEffect(() => {
    if (incidents.length > 0 && !activeIncident) {
      const critical =
        incidents.find(
          (i) =>
            (i as unknown as Record<string, string>).severity === "critical",
        ) ?? incidents[0];
  ...
  const markers: MapMarker[] = [
    ...
    ...(layers.incidents
      ? incidents.map((i) => ({
          id: String(i.id),
          lat:
            (i as unknown as Record<string, number>).lat ??
            i.location_lat ??
            23.0269,
          lng:
            (i as unknown as Record<string, number>).lng ??
            i.location_lon ??
            72.5855,
          ...
          severity:
            i.severity === "medium"
              ? ("moderate" as const)
              : (i.severity as "critical" | "high" | "low"),
          label: String(i.id).slice(0, 8).toUpperCase(),
          popup:
            (i as unknown as Record<string, string>).title ??
            i.description ??
            i.severity,
        }))
      : []),
    ...
    <main className="h-screen relative overflow-hidden pt-16">
        ...
        <div className="relative">
          <div className="bg-white rounded-xl shadow-2xl p-2 flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${showFilters ? "bg-primary text-white" : "hover:bg-surface-container-low text-primary"}`}
            >
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl p-4 z-30">
              ...
              <div className="flex flex-wrap gap-2">
                {["Critical", "High", "Moderate", "Low"].map((s) => (
                  <button
                    key={s}
                    className="px-3 py-1 text-xs font-bold rounded-full bg-surface-container-low text-on-surface hover:bg-primary hover:text-white transition-colors"
                  >
                    {s}
                  </button>
                ))}
              </div>
              <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-3 mt-4">
                Incident Status
              </p>
              <div className="flex flex-wrap gap-2">
      ...
      <div className="absolute top-20 right-6 z-20 flex flex-col gap-3">
        ...
        <div className="bg-white rounded-xl shadow-2xl p-1 flex flex-col items-center">
          <button
            ...
            className="p-3 hover:bg-surface-container-low rounded-lg transition-colors text-on-surface-variant"
            title="Zoom in"
          >
          ...
          <div className="w-8 h-px bg-outline-variant/30"></div>
          <button
            onClick={() => mapHandleRef.current?.flyTo([72.5855, 23.0269], 13)}
            className="p-3 hover:bg-surface-container-low rounded-lg transition-colors text-on-surface-variant"
            title="Reset view"
          >
        ...
        <div className="bg-white rounded-xl shadow-2xl p-2 space-y-1 w-56">
          <h3 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest px-2 py-1">
            Map Layers
          </h3>
          {LAYER_CONTROLS.map(({ key, icon, label }) => (
            ...
            >
              ...
              <div
                className={`w-8 h-4 rounded-full relative transition-colors ${layers[key] ? "bg-primary" : "bg-outline-variant"}`}
              >
                <div
                  className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${layers[key] ? "right-0.5" : "left-0.5"}`}
                ></div>
              </div>
            </button>
          ))}
        ...
        <div className="bg-white rounded-xl shadow-2xl p-4 w-56">
          ...
          <div className="space-y-2">
            {[
              ...
              ...ROUTE_COLORS.slice(0, 3).map((c, i) => ({
                color: c,
                label: `Diversion Route ${i + 1}`,
              })),
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-20">
        <div className="bg-white rounded-full shadow-2xl px-5 py-2 flex items-center gap-3">
          <span
            className={`w-2.5 h-2.5 rounded-full ${incidents.length > 0 ? "bg-error animate-pulse" : "bg-primary"}`}
          ></span>
          <span className="text-xs font-bold text-on-surface">
            {
              incidents.filter(
                (i) =>
                  (i as unknown as Record<string, string>).status === "active",
              ).length
            }{" "}
            Active Incidents
          </span>
          ...
          <span
            className={`text-[10px] font-bold ${backendOnline ? "text-primary" : "text-on-surface-variant"}`}
          >
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {routeOptions.map((r, i) => (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-on-surface leading-tight mb-1 truncate">
                    {r.label}
                  </p>
                  <p className="text-[9px] text-on-surface-variant">
                    +{r.extra_minutes.toFixed(1)} min ·{" "}
                    {r.redistribution.toFixed(0)}% shift
                  </p>
                </button>
              ))}
            </div>
        ...
        <div className="absolute bottom-20 left-6 z-20">
          <div className="bg-white rounded-xl shadow-2xl p-4 w-72">
            ...
            <p className="text-[11px] text-on-surface-variant leading-relaxed mb-2">
              {(activeIncident as unknown as Record<string, string>)
                .description ?? "Incident under monitoring"}
            </p>
            <div className="flex gap-2">
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${activeIncident.severity === "critical" ? "bg-error-container text-on-error-container" : "bg-primary/10 text-primary"}`}
              >
      ...
      {routeOptions.length === 0 && incidents.length > 0 && (
        <SignalActionsPanel
          incidentId={String((activeIncident ?? incidents[0]).id)}
...
function SignalActionsPanel({
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://localhost:8000"}/recommendations/${incidentId}`,
      { signal: AbortSignal.timeout(8000) },
    )
      .then((r) => r.json())
      .then((recs) => {
        const sa = recs.flatMap(
          (r: BackendRecommendation) =>
            r.copilot_response?.signal_actions ?? [],
        );
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-4">
        ...
        <div className="space-y-2">
          {actions.map((a, i) => (
            <div
              key={i}
              className="flex items-start gap-3 p-2 bg-surface-container-low rounded-lg"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px] text-primary mt-0.5">
                  {a.expected_impact} · {Math.round(a.confidence * 100)}%
                  confidence
                </p>
              </div>
            </div>
